...
public class JDBCEmployeeDAO implements EmployeeDAO {
	...
	public Employee findById(Long id) {
		...
		try (ResultSet rs = connection.createStatement()
				.executeQuery("select * from employee where employee_id = " + id)) {
		...
		try {
			...
			if (model.getId() > 0) {
				ps = connection.prepareStatement(
						"update employee set first_name=?, last_name=?, job_title=?, date_of_birth=?, employment_date=?, gender = ? "
								+ "where employee_id = ? returning employee_id");

			} else {
				ps = connection.prepareStatement(
						"insert into employee (first_name, last_name, job_title, date_of_birth, employment_date, gender) "
								+ "values (?, ?, ?, ?, ?, ?) returning employee_id");

			}
			ps.setString(1, model.getFirstName());
			ps.setString(2, model.getLastName());
			ps.setString(3, model.getJobTitle().toString());
	...
	public boolean delete(Employee model) {
		...
		try {
			...
			result = statement.execute("delete from employee where employee_id = " + model.getId());
			connection.commit();
		} catch (SQLException ex) {
